...
(defn PlayerMoveEvent
  ...
  #_ (when (and (= 0 (rand-int 10))
    ...
    (let [x (-> event .getTo .getX)
      ...
      (cond
        (or (< x -300) (< 300 x))
        (l/send-message player (format "Your current x is %d. Go back within -300 <= x <= 300" (int x)))

        (or (< z -300) (< 300 z))
        (l/send-message player (format "Your current z is %d. Go back within -300 <= z <= 300" (int z)))))))
